...
| **Reconcile** | Reconciling N pages… | Image-based merge where run1 ≠ run2 (content) |
...
| Colonial bilingual PDF (Spanish + Nahuatl, Kaqchikel, etc.) | **PDF Transcribe** (v3 auto-detect + reconcile + spot patches) |
| Need pixel-level flag review UI (J/K flags, accept/reject spans) | **Archive Studios** |
| Need full-book verbatim transcription with 2-pass + reconcile | **PDF Transcribe** |
| Selling to customers with Gumroad page credits | **Archive Studios** + Gateway |
| Personal research PDFs with your own Claude API key | **PDF Transcribe** |
| Already ran OCR in Archive Studios and want a second opinion on a PDF | **PDF Transcribe** (separate pipeline; does not import Archive output) |
